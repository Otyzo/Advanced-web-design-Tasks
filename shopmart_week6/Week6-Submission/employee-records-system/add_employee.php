<?php
// add_employee.php - Create operation
require_once 'config.php';
require_once 'includes/auth.php';

$errors = [];
$full_name = $email = $phone = $department = $position = $salary = $date_hired = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare(
            "INSERT INTO employees (full_name, email, phone, department, position, salary, date_hired)
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
        $stmt->bind_param("sssssds", $full_name, $email, $phone, $department, $position, $salaryValue, $date_hired);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Database error: could not save the employee record.";
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <title>Add Employee - Employee Records System</title>
...
<body class="bg-slate-50 min-h-screen">
    ...
    <main class="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <h2 class="text-xl font-semibold text-slate-800 mb-6">Add New Employee</h2>
        ...
        <form method="POST" action="add_employee.php" class="bg-white rounded-xl shadow-sm p-6 space-y-4">
